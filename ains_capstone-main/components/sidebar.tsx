...
import { LayoutDashboard, Users, MessageSquare, BarChart3, Settings, LogOut, Shield } from "lucide-react";
...
export default function Sidebar() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        localStorage.removeItem("ains_role");
        localStorage.removeItem("ains_email");
        router.push("/login");
      }
    })();
  }, [router]);
  ...
  const navigation = useMemo(() => {
    ...
    if (isAdmin) base.unshift({ name: "Admin", href: "/admin", icon: Shield });
    return base;
  }, [isAdmin]);
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive ? "bg-[#E8B86D] text-[#0C1D40] font-semibold" : "text-gray-300 hover:bg-white/10 hover:text-white"
              )}
            >
